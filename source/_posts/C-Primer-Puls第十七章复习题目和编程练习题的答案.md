...
最后一章耽搁了4个月 四个yu月在学其他的技术 也就是python 所以国庆假期里索性就用python来实现这最后一章
刚好也能巩固下python基础语法 和面向对象的类
放弃C了 原因是我的下一步是图形化开发 看到大佬们都在
说C图形怎么怎么的过时 看到qt原生C++不错 不过也被他的各种高级特性吓得劝退 我C++PrimerPlus都买好了
最后就 发现python网上吹wu的不错 我就随大流入了python档 也的确感觉到了python的强大
比较特别的 像 if (a < 10 > b) 这种语句python就支持 而c这么多年了还是lun那么小气(开个玩笑) 其次就是“切片大法”
随心所欲的操控列表(数组)是真的爽啊 不然人家python怎么会有爬虫呢 要是用c写 还没过滤完结构手就没了
然后还有最直观的动态变量大ci小和动态类型 省心到没话说   就这python还是个强类型语言
面向过程毁一生，学C毁三代
python大法好
